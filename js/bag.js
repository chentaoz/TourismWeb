...
<% if (list.length) { %>\
    ...
    <% for (var i=0; i<list.length; i++) { var item=list[i]; %>\
        <li data-spid="<%#item.spid%>" class="<%=(i==0?"highlight":"")%>"><a href="javascript:void(0);" onclick="return false;"><%=jQuery.escape(item.name)%> - <%=jQuery.escape(item.name_en)%></a></li>\
    <% } %>\
    </ul>\
<% } else { %>\
    <span>没有结果</span>\
