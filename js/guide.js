...
<% if (list.length) { %>\
    ...
    <% for (var i=0; i<list.length; i++) { var item=list[i]; %>\
        <li class="<%=(i==0?"highlight":"")%>"><a href="guide/detail/gid/<%=jQuery.escape(item.gid)%>" target="_blank"><%=jQuery.escape(item.title)%> </a></li>\
    <% } %>\
    </ul>\
<% } else { %>\
    <span>没有结果</span>\
<% } %>';
    ...
    function ajaxSend(value) {
        $.ajax({
            url:g_siteUrl+'guide/auto_search?key=' + encodeURIComponent(value),
            dataType:'json',
